...
//		$log->logInfo("response data:" . $map->toString());
	...
	private static function getNotifyRequestPlain($map) {
		...
		$trade_no = StringUtil::trim($map->get("trade_no"));
		$pay_date = StringUtil::trim($map->get("pay_date"));
		$pay_type = StringUtil::trim($map->get("pay_type"));
		$amount = StringUtil::trim($map->get("amount"));
		$amt_type = StringUtil::trim($map->get("amt_type"));
		$settle_date = StringUtil::trim($map->get("settle_date"));
		$trade_state = StringUtil::trim($map->get("trade_state"));
		$version = StringUtil::trim($map->get("version"));
		...
		$data->put("settle_date",$settle_date);
		$data->put("trade_state",$trade_state);
		$data->put("pay_date",$pay_date);
		$data->put("amt_type",$amt_type);
		$data->put("pay_type",$pay_type);
		$data->put("version",$version);
		if(!is_null($map->get("goods_id"))){
			$data->put("goods_id",StringUtil::trim($map->get("goods_id")));
		}
		if(!is_null($map->get("pay_seq"))){
			$data->put("pay_seq",StringUtil::trim($map->get("pay_seq")));
		}
		if(!is_null($map->get("error_code"))){
			$data->put("error_code",StringUtil::trim($map->get("error_code")));
		}
		if(!is_null($map->get("mer_priv"))){
			$data->put("mer_priv",StringUtil::trim($map->get("mer_priv")));
		}
		if(!is_null($map->get("media_id"))){
			$data->put("media_id",StringUtil::trim($map->get("media_id")));
		}
		if(!is_null($map->get("media_type"))){
			$data->put("media_type",StringUtil::trim($map->get("media_type")));
		}
