...
<form role="form" class="row">
        ...
        <ul class="date">
            ...
            for($i = $start; $i <= $end; $i = $i + 86400)
            {
                ...
                if (isset($date[$i]))
                {
                    ...
                    $s = json_decode($d['standby'], true);
                    $breakfasts = array('无早', '单早', '双早', '三早');
                    $bf = $d['breakfast'] > 3 ? "{$d['breakfast']}早" : $breakfasts[$d['breakfast']];

                    if (!$d['filled'] && ($d['allot']-$d['sold'] > 0))
                    {
                        $price = "<span class=\"label label-success hidden-xs\">{$d['price']}<em>{$bf}</em></span>";
                        $class .= 'allot ';
                    }
                    else if (!$d['filled'])
                    {
                        $price = "<span class=\"label label-warning hidden-xs\">{$d['price']}<em>{$bf}</em></span>";
                        $class .= 'request ';
                    }
                    else
                    {
                        $price = "<span class=\"label label-danger hidden-xs\">{$d['price']}<em>{$bf}</em></span>";
                        $class .= 'filled ';
                    }

                    $price .= "<input type=\"hidden\" name=\"price[{$i}]\" value=\"{$d['price']}\" />";
                    $price .= "<input type=\"hidden\" name=\"breakfast[{$i}]\" value=\"{$d['breakfast']}\" />";
                    $price .= "<input type=\"hidden\" name=\"child[{$i}]\" value=\"{$s['child']}\" />";
                    $price .= "<input type=\"hidden\" name=\"baby[{$i}]\" value=\"{$s['baby']}\" />";
                ...
                else
                {
                    ...
                    $price .= "<input type=\"hidden\" name=\"breakfast[{$i}]\" value=\"0\" />";
            ...
            ?>
            <li class="<?php echo trim($class); ?>" data-date="<?php echo $i; ?>" data-sold="<?php echo $d['sold']; ?>"><b><?php echo date('j', $i); ?></b><?php echo $price; ?></li>
            <?php } ?>
        </ul>
    ...
    <div id="calendar-setting" class="col-sm-3 setting">
        ...
        <div class="form-group">
            <label>含早餐</label>
            <div class="input-group">
                <input type="text" class="form-control" id="date-breakfast" placeholder="" value="" />
...
$(function(){
    ...
    $("#calendar-setting .save").click(function(){
        var data = {price:"", breakfast:"", child:"", baby:"", allot:""};
        var name = {price:"房价", breakfast:"早餐数", child:"儿童附加费", baby:"婴儿附加费", allot:"库存"};
        var i = 0;
        for(x in data){
            var _t = $("#date-"+x).val();
            if (x == "allot" && _t == "") {
            ...
            if (_t != ""){
                ...
                $(".calendar .date .selected:not(.freeze)").each(function(){
                    ...
                    if (x == "price" || x == "breakfast") {
                        var bf = li.children("input:eq(1)").val();
                        var breakfast = "";
                        switch(bf){
                            case "0": breakfast = "无早"; break;
                            case "1": breakfast = "单早"; break;
                            case "2": breakfast = "双早"; break;
                            case "3": breakfast = "三早"; break;
                            default: breakfast = bf + "早";
                        }
                        span.html(p+"<em>"+breakfast+"</em>").show();
                    }
                    else if (x == "allot") {
                        li.removeClass("allot request filled");
    ...
    $(".calendar .date li b").popover({trigger:"hover", placement:"top", container:"#item-price", html:true, content:function(){
        ...
        var d = {price:inp.eq(0).val(), breakfast:inp.eq(1).val(), child:inp.eq(2).val(), baby:inp.eq(3).val(), allot:inp.eq(4).val()};
        if (d.price == "0") return "";
        ...
        switch(d.breakfast){
            ...
            case "2": breakfast = "双早"; break;
            case "3": breakfast = "三早"; break;
            default: breakfast = d.breakfast + "早";
        }
        if (d.price) {
            content += "房价："+d.price+"<br />";
            content += "早餐："+breakfast+"<br />";
