...
class hotel_flight_prepay_hook
{
    ...
    public function __construct()
    {
        ...
        filter::add('order_flight',                 array($this, 'order_book_flight'), 10, 5);
    ...
    public function __call($method, $args=array())
    {
        ...
        if (false !== strpos($method, 'api'))
        {
            ...
            if (!$api) $api = new hotel_flight_prepay_api();
        ...
        else if (false !== strpos($method, 'order'))
        {
            ...
            if (!$order) $order = new hotel_flight_prepay_order();
    ...
    {
        if ($type != 4 || $payment != 'prepay') return;
        ...
        if ($itemid)
        {
            switch ($itemtype)
            {
                ...
                case 'flight':
                    $sql = "SELECT p.*, h.`code` AS `flight_name`
                            FROM `ptc_product_item` AS p
                                LEFT JOIN `ptc_flight` AS h ON p.`objpid` = h.`id`
                            WHERE p.`id`=:id;";
                    $data = $db -> prepare($sql) -> execute(array(':id' => $itemid));
                    if (!$data) break;
        ...
        switch ($itemtype)
        {
            ...
            case 'flight':
                $sql = "SELECT a.`id`, a.`code`, b.`name` AS `depart_airport`, c.`name` AS `arrive_airport`
                        FROM `ptc_flight` AS a
                            LEFT JOIN `ptc_flight_airport` AS b ON a.`depart` = b.`code`
                            LEFT JOIN `ptc_flight_airport` AS c ON a.`arrive` = c.`code`";
                $flights = $db -> prepare($sql) -> execute();
            break;
        }
    ...
    {
        if ($product['type'] != 4 || $product['payment'] != 'prepay' || !in_array($type, array('hotel', 'flight'))) return $data;
        ...
        switch ($type)
        {
            ...
            case 'flight':
                $data['objtype']     = 'flight';
                $data['objpid']      = (int)$_POST['flight'];
                $data['objid']       = (int)$_POST['back'];
                $data['ext']         = (int)$_POST['ext'];       // back day
                $data['ext2']        = (string)$_POST['class'];  // class
                $data['data']       = array(
                        'advance'       => (int)$_POST['advance'],
                        'meal'          => (string)$_POST['meal'],
                );

                if (!$data['objpid']) json_return(null, 1, '未选择航班');
                if (!$data['ext2'])   json_return(null, 1, '请输入舱位类型');
                if (!$data['objid'])  $data['ext'] = 0;

                $sql = "SELECT a.`code`, b.`city` AS `source`, c.`city` AS `target`
                        FROM `ptc_flight` AS a
                            LEFT JOIN `ptc_flight_airport` AS b ON a.`depart` = b.`code`
                            LEFT JOIN `ptc_flight_airport` AS c ON a.`arrive` = c.`code`
                        WHERE a.`id`=:id";
                ...
                $db = db(config('db'));
                $flight = $db -> prepare($sql) -> execute(array(':id' => $data['objpid']));
                if (!$flight) json_return(null, 1, '航班不存在');

                $data['source'] = $flight[0]['source'];
                $data['target'] = $flight[0]['target'];

                $this -> save_data = array('flight' => $flight[0]['code']);
            break;
        }
    ...
    {
        if ($item['product_type'] != 4 || $item['product_payment'] != 'prepay') return;
        ...
        if ($_POST)
        {
            ...
            switch ($item['objtype'])
            {
                ...
                case 'flight':
                    $data = $this -> _format_flight_price($item);

                    // close old price
                    $where = "`supply`='EBK' AND `supplyid`=:sup AND `payment`=3 AND `package`=:package AND (`flight`=:flight1 OR `flight`=:flight2) AND `date`>=:start AND `date`<=:end";
                    $condition = array(':sup'=>$item['pid'], ':package'=>$item['id'], ':flight1'=>$item['objpid'], ':flight2'=>$item['objid'], ':start'=>$_POST['start'], ':end'=>$_POST['end']);
                    $rs = $db -> prepare("UPDATE `ptc_flight_price_date` SET `close`=1, `price`=0 WHERE {$where}") -> execute($condition);
                    if ($rs === false)
                        json_return(null, 6, '保存失败，请重试');
                    ...
                    if ($data)
                    {
                        ...
                        $_columns = update_column(array_keys($data[0]));
                        $rs = $db -> prepare("INSERT INTO `ptc_flight_price_date` {$column} VALUES {$sql} ON DUPLICATE KEY UPDATE {$_columns};") -> execute($value); //var_dump($rs); exit;
                        if (false == $rs)
                        {
                            $db -> rollback();
                            json_return(null, 7, '数据保存失败，请重试~');
        ...
        switch ($item['objtype'])
        {
            ...
            case 'flight':
                $_date = $db -> prepare("SELECT `key`,`date`,`price`,`child`,`baby`,`adultfuel`,`childfuel`,`babyfuel`,`adulttax`,`childtax`,`babytax`,`allot`,`sold`,`filled`,`close` FROM `ptc_flight_price_date` WHERE `supply`='EBK' AND `supplyid`=:sup AND `package`=:package AND `flight`=:flight AND `date`>=:start AND `date`<=:end AND `close`=0")
                            -> execute(array(':sup'=>$item['pid'], ':package'=>$item['id'], ':flight'=>$item['objpid'], ':start'=>$start, ':end'=>$end));
                break;
        }
    ...
    // _format_flight_price
    private function _format_flight_price($item)
    {
        $data = array();
        ...
        foreach ($_POST['price'] as $date => $price)
        {
            ...
            $flights = array($item['objpid']);
            if ($item['objid']) $flights[] = $item['objid'];

            foreach ($flights as $k => $flight)
            {
                $back = $item['objid'] ? (int)$item['ext'] : 0;
                // KEY : date(6), flight(6), pay(1), booktime(2), class(1), package(6), back(2), supply(3), supply no.(-)
                $key   = date('ymd', $date)
                        .str_pad(strtoupper(dechex($flight)), 6, 0, STR_PAD_LEFT)
                        .'200'.$item['ext2']
                        .str_pad(strtoupper(dechex($item['id'])), 6, 0, STR_PAD_LEFT)
                        .str_pad(strtoupper(dechex($back)), 2, 0, STR_PAD_LEFT)
                        .'EBK'.$item['pid'];
                ...
                $data[$key] = array(
                    ...
                    'supplyid'  => $item['pid'],
                    'advance'   => (int)$extend['advance'],
                    'meal'      => (int)$extend['meal'],
                    'child'     => (int)$_POST['child'][$date],
                    'baby'      => (int)$_POST['baby'][$date],
                    'adultfuel' => (int)$_POST['adultfuel'][$date],
                    'childfuel' => (int)$_POST['childfuel'][$date],
                    'babyfuel'  => (int)$_POST['babyfuel'][$date],
                    'adulttax'  => (int)$_POST['adulttax'][$date],
                    'childtax'  => (int)$_POST['childtax'][$date],
                    'babytax'   => (int)$_POST['babytax'][$date],
                    'back'      => $k == 0 ? $back : -$back,
                    'filled'    => $allot < 0 ? 1 : 0,
                    'allot'     => $allot < 0 ? 0 : $allot,
                    'cutoff'    => 0,
