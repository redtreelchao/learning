...
class hotel_flight_prepay_api extends hotel_flight_prepay_hook
{
    ...
    public function search_condition($sql, $types, $checkin, $checkout, $min, $max)
    {
        if (in_array('flight', $types))
        {
            $sql[0] .= "LEFT JOIN `ptc_product_item` AS di ON di.`objtype`='room' AND di.`objpid` = h.`id` "
                    ...
                    ."LEFT JOIN `ptc_product` AS d ON di.`pid` = d.`id` AND d.`type`=4 AND d.`payment`='prepay' AND d.`status` = 1";
    ...
    {
        if (!in_array('flight', $types)) return $hotel;
        ...
        $sql = "SELECT d.`type`, d.`payment`, d.`id`, d.`name`, d.`intro`, d.`rule`, d.`minprice`, d.`maxprice`, d.`updatetime`,
                ...
                FROM `ptc_product_item` AS di
                    LEFT JOIN `ptc_product` AS d ON di.`pid` = d.`id` AND d.`status` = 1
                        ...
                        LEFT JOIN `ptc_org_profit` AS fd2 ON fd2.`org` = :org AND fd2.`payment` = 'prepay' AND fd2.`objtype` = 'product4' AND fd2.`objid` = d.`id`
                WHERE d.`type`=4 AND di.`objtype`=:type AND di.`objpid` = :id"
                .($checkin && $checkout ? ' AND di.`start` <= :checkin AND di.`end` > :checkout' : '')
    ...
    {
        if ($product['type'] != 4 || $product['payment'] != 'prepay') return $product;

        // type = 1 only hotel + flight product
        ...
        foreach ($items as $k => $v)
        {
            ...
            {
                $items[$k]['flight']  = $v['objpid'];
                $items[$k]['return']  = $v['objid'];
                $items[$k]['class']   = $v['ext2'];

                $sql = "SELECT p.`key` AS `code`, p.`date`, p.`price`, p.`child`, p.`baby`, p.`adultfuel`, p.`childfuel`, p.`babyfuel`, p.`adulttax`, p.`childtax`, p.`babytax`, p.`allot`-p.`sold` AS `allot`, p.`filled`
                        FROM `ptc_flight_price_date` AS p
                        WHERE `supply`='EBK' AND `supplyid`=:sup AND `package`=:package AND `flight`=:flight AND `close`=0";
                $condition = array(':sup'=>$product['id'], ':package'=>$v['id'], ':flight'=>$v['objpid']);
            ...
            foreach ($date as $d)
            {
                $d['code'] = key_encryption($d['code'].'_product4');
                ...
                if ($v['type'] == 'room')
                {
                    $hmin = $hmin > $d['price']/2 ? $d['price']/2 : $hmin;
                    $hmax = $hmax < $d['price']/2 ? $d['price']/2 : $hmax;
                }
                else
                {
