...
<head>
    ...
    <title>PUTIKE &rsaquo; 深捷旅国籍列表</title>
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-sm-11 col-sm-offset-1 col-md-10 col-md-offset-2 main">
                ...
                <h1 class="page-header">深捷旅国籍列表</h1>
                    ...
                    <form class="col-xs-4 col-sm-6 col-md-6 col-lg-8 form-inline" action="" method="GET" role="form">
                        ...
                        <div class="btn-group" style="margin:20px 0px; margin-right:10px;">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li><a href="<?php echo BASE_URL; ?>jlt.php?method=nation&status=1">显示已关联</a></li>
                                <li><a href="<?php echo BASE_URL; ?>jlt.php?method=nation&status=0">显示未关联</a></li>
                            </ul>
                        </div>
                        <!-- end filter -->
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            <tr id="row-<?php echo md5($v['code']); ?>">
                                <td><input type="checkbox" class="checkbox" value="<?php echo $v['code']; ?>" /></td>
                                <td><?php echo $v['code']; ?></td>
                                <td><?php echo $v['name']; ?><?php if($v['new']){ ?> <span class="label label-danger label-xs">NEW</span><?php } ?></td>
                                <td>
                                    <?php if($v['bind']) { ?>
                                ...
                                <td>
                                    <a class="btn btn-sm btn-default" href="javascript:bind('<?php echo $v['code']; ?>', '<?php echo md5($v['code']); ?>')"><span class="glyphicon glyphicon-link hidden-md"></span><span class="hidden-xs hidden-sm"> 关联</span></a>
                                    <a class="btn btn-sm btn-default" href="javascript:;" onclick="unbind('<?php echo $v['code']; ?>', '<?php echo md5($v['code']); ?>', this)"><span class="glyphicon glyphicon-ok hidden-md"></span><span class="hidden-xs hidden-sm"> 不关联</span></a>
                                </td>
                            </tr>
                            <?php } ?>
                        </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">搜索国籍要求</h4>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-8">
                            <div class="input-group">
                                <input type="text" name="keyword" class="form-control" placeholder="地区/国家" autocomplete="off" value="" />
                                <span class="input-group-btn">
    ...
    <script>
    function bind(code, md){
        $("#bind-search").modal('show');
        var li = $("#row-"+md);
        var name = li.children("td:eq(2)").clone();
        name.children(".label").remove();
        name = $.trim(name.text());
        $("#bind-search .bind-search-ul").html("<div style='text-align:center; color:#999;'>正在加载自动匹配内容..</div>");
        console.log(name);
        search(1, name);
        ...
        $("#bind-search .modal-footer .btn-primary").unbind("click").bind("click", function(){
            ...
            //console.log(code);
            $.post("<?php echo BASE_URL; ?>jlt.php?method=nation", {code:code,bind:bind}, function(data){
                if (data.s == 0) {
                    var label = $('<span class="label label-primary" data-toggle="tooltip" data-placement="top" title="">已关联</span>');
                    label.attr("title", name);
                    $("#row-"+md+" td:eq(3)").html(label);
                    $("#row-"+md+" td:eq(2)").find(".label").remove();
    ...
    function unbind(code, md, btn){
        $(btn).children(".glyphicon").removeClass("glyphicon-ok").addClass("glyphicon-refresh glyphicon-loading");
        $.post("<?php echo BASE_URL; ?>jlt.php?method=nation", {code:code,bind:0}, function(data){
    ...
    function search(page, bind){
        var btn = $("#bind-search .search");
        if(page === undefined) page = 1;
        if(bind === undefined){
            bind =  btn.parent().prev("input").val();
        }
        if (!bind) {
            alert("请输入查询关键词", "error", null, "#bind-search .modal-body");
        }
        btn.html('<span class="glyphicon glyphicon-refresh glyphicon-loading"></span>').prop("disabled", true);
        $.get("<?php echo BASE_URL; ?>nation.php", {keyword:'', bind:bind, page:page}, function(data){
            if (data.s == 0){
                ...
                if(data.rs.list.length > 0){
                    ...
                    ul.append("<div style='text-align:center; color:#999;'>未找到任何数据，请尝试其他关键词或<a href=\"<?php echo BASE_URL; ?>nation.php#new\" target=\"_blank\">添加</a></div></div>");
                }
