...
if($page)
{
    ...
    echo '<li'.($page['now'] - ceil($page['show']/2) > 0 ? '' : ' class="disabled"')."><a href=\"{$page['url']}1\">&laquo;</a></li>";
    ...
    for ($i = $start; $i <= $end; $i++)
    {
        ...
        echo "<li class=\"{$class}\"><a href=\"{$page['url']}{$i}\">{$i}</a></li>";
    }

    echo '<li class="hidden-xs"><span><input value="" data-url="'.$page['url'].'" placeholder="Go" /></span></li>';

    echo '<li'.($page['now'] + ceil($page['show']/2) <= $page['total'] ? '' : ' class="disabled"')."><a href=\"{$page['url']}{$page['total']}\">&raquo;</a></li>";
